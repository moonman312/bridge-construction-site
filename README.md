...
Audience-specific landing site for **hotelbridgeloans.com**, bundling the Hotel Capital Stack diagnostic. Static — no build step.
...
```bash
git init && git add . && git commit -m "Hotel Bridge & Construction Loans site"
git branch -M main
gh repo create moonman312/bridge-construction-site --public --source=. --remote=origin --push
vercel && vercel --prod
```

## Domains for this project (add in Vercel → Settings → Domains)
- Primary: **hotelbridgeloans.com**
- hotelconstructionfinancing.com → set to **Redirect to** hotelbridgeloans.com
